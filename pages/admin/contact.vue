...
const fetchAllPersons = async () => {
  const response = await findAll({
    paging: { limit: limit.value, offset: offset.value }
  })
  personsData.value = response.result?.data?.findManyContacts?.items as any
  totalCount.value = response.result?.data?.findManyContacts?.count as number
}
